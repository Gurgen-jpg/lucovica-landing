...
export default function SugaringPricing() {
  const [activeTab, setActiveTab] = useState('sugaring-women')
    ...
    <>
      <section id="pricing" className="py-20 px-5 md:px-12 bg-mist/20">
        <div className="max-w-4xl mx-auto">
          <SectionHeader
            sub="Прейскурант"
            title="Цены на шугаринг"
            description="Все цены указаны за один сеанс."
            spacing="sm"
          />

          <div className="flex flex-wrap gap-2 mb-8 justify-center">
            {SUGARING_TABS.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`text-xs sm:text-sm font-semibold px-4 py-2 rounded-full border-2 transition-all ${
                  activeTab === tab.id ? 'bg-dark border-dark text-white' : 'border-mist text-dark/60 hover:border-dark'
                }`}
              >
                {tab.label}
              </button>
          ...
          {category && (
            <div className="bg-white rounded-2xl border border-mist overflow-hidden shadow-sm">
              <div className="flex items-center justify-between px-5 py-4 border-b border-mist">
                ...
                <button onClick={() => handleBook(category.title)} className="btn-primary text-xs px-4 py-2">
                  Записаться
                </button>
              </div>
